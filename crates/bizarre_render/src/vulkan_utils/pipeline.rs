use std::{ffi::CStr, mem::size_of, path::Path};
...
pub fn create_test_pipeline(
    ...
    let (vert_module, vert_stage) = {
        let code = load_shader(Path::new("assets/shaders/test.vert"), ShaderType::Vertex)?;
    ...
    let (frag_module, frag_stage) = {
        let code = load_shader(Path::new("assets/shaders/test.frag"), ShaderType::Fragment)?;
...
pub fn create_deferred_pipeline(
    ...
    let vertex_input_attributes = Vertex::attribute_description();
    ...
    let set_layout = {
        let set_bindings = deferred::descriptor_set_bindings();
